...
public class Server {
	...
	private JTextArea contentArea;//内容显示区域
	private JTextField txt_message;//输入的内容
	private JTextField txt_max;//输入的内容最大长度
	private JTextField txt_port;//服务器的端口号
	private JButton btn_start;//服务器启动按钮
	private JButton btn_stop;//服务器停止按钮
	...
	private boolean isStart = false;//声明一个值来判断服务器是否开启
...
//		frame.setIconImage(Toolkit.getDefaultToolkit().createImage(Server.class.getResource("qq.png")));
	...
	class ClientThread extends Thread{
					...
					if(message.equals("CLOSE")){
						...
						for(int i = clients.size()-1;i>=0;i--){
							...
							if(clients.get(i).getUser() == user){
								ClientThread temp = clients.get(i);//获取该用户的线程
		...
		public void dispatcherMessage(String message){
			//使用StringTokenizer将message分解为标记
